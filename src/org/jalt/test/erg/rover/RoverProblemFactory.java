...
public class RoverProblemFactory extends ProblemFactory {
	...
	public RoverProblemFactory(final int pRows, final int pCols, final int pAgents, final int pNumberOfObstacles) {
		rows = pRows;
		cols = pCols;
		agents = pAgents;
	...
	public Problem<ERG> doCreate() {
		// create model
		final RoverModel model = new RoverModel(rows, cols);
		...
		model.setRewardFunction(ToRL.convertRewardFunction(model, BAD_REWARD, RoverModel.getBadRewardObstacles()));
