 ...
 * @author And
 */
public class HunterPreyProblemFactory extends ProblemFactory<HunterPreyModel> {
	...
	@Override
	protected Problem<HunterPreyModel> doCreate() {
		final HunterPreyModel model = new HunterPreyModel(rows, cols);
		...
		model.setRewardFunction(new RewardFunctionState<MDP>(model, CollectionsUtils.createMap(finalStates, CAPTURE_REWARD), OTHERWISE));

		return new Problem<HunterPreyModel>(model, CollectionsUtils.asIndexMap(initStates), finalStates);
	}
}
