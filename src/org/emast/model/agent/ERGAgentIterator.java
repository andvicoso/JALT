...
public class ERGAgentIterator<M extends ERG> extends AgentIterator<M> {
	...
	public ERGAgentIterator(int pNumber) {
		this(pNumber, Collections.<Individual<M>> emptyList());
	}
	...
	public Plan run(Problem<M> pProblem, Map<String, Object> pParameters) {
		...
		q = new QTable<QTableItem>(model.getStates(), model.getActions());
		Action action;
			...
			do {
				...
				if (action != null) {
					...
					if (nextState != null) {
						...
						// behave(RewardBehavior.class, pProblem, "state", nextState, "reward",
						// reward);
	...
	private void behave(Class<? extends Individual> pClass, Problem problem,
