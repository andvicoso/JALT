...
public class ChangePreservGoal implements Individual<ERG>, ChangeModel<ERG> {
	...
	@Override
	public void behave(ERGAgentIterator pAgent, Problem<ERG> pProblem,
	...
	protected Problem<ERG> cloneProblem(ERG pModel, State pState, int pAgent,
			Expression pNewPreservGoal) {
	...
	protected Policy changePreservationGoal(ERGAgentIterator pAgent, ERG pModel, State pState)
		...
		Collection<Proposition> props = pModel.getPropositionFunction().getPropositionsForState(
				pState);
		...
		if (!newPreservGoal.equals(originalPreservGoal)
			...
			// Execute the base algorithm (PPFERG) over the new problem (with the new preservation
			// goal)
			Policy p = algorithm.run(newProblem, Collections.<String, Object> emptyMap());
			...
			if (ValidPathFinder.exist(newProblem, pAgent.getPolicy(), number)) {
				...
				Log.info("changed preservation goal from {" + originalPreservGoal + "} to {"
						+ newPreservGoal + "}");
				return p;
			}
		}
