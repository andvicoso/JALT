...
import org.emast.model.algorithm.planning.agent.behaviour.Behaviour;
import org.emast.model.algorithm.planning.agent.behaviour.RewardBehaviour;
 ...
 * @author Anderson
 */
public class AgentIteratorBehaviour<M extends MDP> implements Algorithm<M, Plan> {
    ...
    public AgentIteratorBehaviour(final int pAgent) {
        agent = pAgent;
        behaviours = new Lookup();
    }

    public Lookup getBehaviours() {
        return behaviours;
    }

    public void setBehaviours(Lookup behaviours) {
    ...
    protected void addReward(State pNextState, double pReward) {
        ...
        executeBehaviours(RewardBehaviour.class, new Lookup(pNextState, pReward));
    }
    ...
    private void executeBehaviours(Class<? extends Behaviour> pClass, Lookup pParameters) {
        if (behaviours.contains(pClass)) {
            for (Behaviour beh : behaviours.getAll(pClass)) {
                beh.execute(pParameters);
            }
        }
    }
}
