 ...
 * @author Anderson
 */
public class AgentEnsemble implements Algorithm<ERG, Policy>,
		PolicyGenerator<ERG> {

	private final PolicyGenerator<ERG> policyGenerator;
	private List<ReinforcementLearning> agentIterators;
	...
	public String printResults() {
		...
		for (ReinforcementLearning agent : agentIterators) {
	...
	public Policy run(Problem<ERG> pProblem, Map<String, Object> pParameters) {
		...
		// start main loop
		do {
			agentIterators = new ArrayList<ReinforcementLearning>(model.getAgents());
			Log.info("\nITERATION " + iterations++ + ":\n");
			// create initial policy
			policy = policyGenerator.run(pProblem, Collections.<String,Object>emptyMap());

			for (int i = 0; i < model.getAgents(); i++) {
				final ReinforcementLearning agentIterator = new QLearning();
	...
	protected boolean changePreservationGoal(Problem<ERG> pProblem,
		...
		// get the new preservation goal, based on the original and bad reward
		// props
		Expression newPreservGoal = new PreservationGoalFactory()
				.createPreservationGoal(originalPreservGoal, pProps);
		...
		if (!newPreservGoal.equals(originalPreservGoal)
			...
			Problem newProblem = new Problem(newModel,
					pProblem.getInitialStates());
			// Execute the base algorithm (PPFERG) over the new model (with new
			// preservation goal)
			// if there are paths for all to reach the goal
			Log.info("Trying to find a valid plan for preserv: "
					+ model.getPreservationGoal());
	...
	private boolean existValidFinalState(ERG model, Expression newPreservGoal) {
		try {
			Collection<State> finalStates = model.getPropositionFunction()
					.intension(model.getStates(), model.getPropositions(),
							model.getGoal());

			for (State state : finalStates) {
				if (model.getPropositionFunction().satisfies(state,
						newPreservGoal)) {
					return true;
				}
			}
	...
	private boolean changePreservGoal(Problem<ERG> pProblem) {
		...
		Chooser<Proposition> chooser = new MinValueChooser<Proposition>();
